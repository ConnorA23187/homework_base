...
from tensorflow.python.keras.layers import Input, Conv2D, MaxPooling2D, Dropout, Conv2DTranspose, concatenate, ZeroPadding2D, Dense
...
def zero_pad_test(target):
    ...
    assert type(x_pad) is np.ndarray, "Output must be a np array"
...
def conv_single_step_test(target):
    ...
    assert (type(Z) is np.float64 or type(Z) is np.float32), "You must cast the output to float"
